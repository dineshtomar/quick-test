...
export class ActivityService {
    ...
    async getProjectActivities(
        ...
        const activities = await this.activityRepository
            ...
            .innerJoinAndSelect("activity.project", "project", "activity.projectId = :projectId", {
                projectId
            })
            .withDeleted()
            .leftJoinAndSelect("activity.user", "user")
            .leftJoinAndSelect("activity.testSuite", "testSuite", "testSuite.deletedAt Is NULL")
            .leftJoinAndSelect("activity.milestone", "milestone", "milestone.deletedAt Is NULL")
     ...
     */
     ...
     async getTestCaseResultActivities(
        ...
        const activities = await this.activityRepository
            ...
            .innerJoinAndSelect("activity.testCaseResult", "testCaseResult", "testCaseResult.deletedAt Is NULL")
            .withDeleted()
            .leftJoinAndSelect("activity.user", "user")
            .where("activity.entity = :entity", {
                entity: ActivityEntityType.TESTCASERESULT
    ...
    removeDuplicateTestCaseResultActivities(
        ...
        for(let j = activitiesLength-1; j >= 0; j--) {
            ...
            const checkStatus = activity.status.match(/PASSED|FAILED|UNTESTED|BLOCKED/);       
    ...
    async getAllTestCaseChangesActivities(
        ...
        return new ActivityListPageDto(result, new PageMetaDto({
            pageOptionsDto,
            itemCount
        }));
    }
     ...
     */
     async getTestCaseChangesActivitiesByProjects(
        ...
        const activities = await this.activityRepository
            .createQueryBuilder("activity")
            .innerJoinAndSelect("activity.project", "project", "activity.projectId IN (:...projectIds)", {
                projectIds
            })
            .innerJoinAndSelect("activity.testCaseResult", "testCaseResult", "testCaseResult.deletedAt Is NULL")
            .where("activity.entity = :entity", {
    ...
    async getProjectsTestChangesCount(
        ...
        const count = await this.activityRepository
            .createQueryBuilder("activity")
            .innerJoinAndSelect("activity.project", "project", "project.deletedAt is Null AND project.id = :projectId", {
                projectId
            })
            .innerJoinAndSelect("activity.testCaseResult", "testCaseResult", "testCaseResult.deletedAt Is NULL")
            .where("activity.entity = :entity", {
    ...
    async updateUserActivtiesByUserId(updateActivites: any): Promise<boolean> {
        const updatedActivities = await this.activityRepository.save(updateActivites);
    ...
    async getTestSuitesActivitiesByTestSuitesId(testSuiteId: string):Promise<any> {
        return await this.activityRepository
