...
export class ActivityEntity extends AbstractEntity<ActivityDto> {
    ...
    @ManyToOne(() => TestCaseResultEntity, testCaseResult => testCaseResult.activities)
    testCaseResult: Relation<TestCaseResultEntity>;

    @ManyToOne(() => ProjectEntity, project => project.activities, { onDelete:"CASCADE" })
    project: Relation<ProjectEntity>;

    @ManyToOne(() => TestSuiteEntity, testSuite => testSuite.activities)
    testSuite: Relation<TestSuiteEntity>;

    @ManyToOne(() => UserEntity, user => user.activities,{ onDelete:"SET NULL" })
