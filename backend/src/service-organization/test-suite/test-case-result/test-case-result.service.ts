...
export class TestCaseResultService {
    ...
    onModuleInit(): void {
        this.testSuiteService = this.moduleRef.get(
            TestSuiteService,
            {
                strict: false
            }
        );
        this.testCaseService = this.moduleRef.get(
            TestCaseService,
            {
                strict: false
            }
        );
        this.activityService = this.moduleRef.get(
            ActivityService,
            {
                strict: false
            }
        );
        this.defectService = this.moduleRef.get(
            DefectService,
            {
                strict: false
            }
    ...
    async createTestCaseResult(
        ...
        testCaseResult.sectionName = testCase.section? testCase.section.name : 'Unassigned';
        testCaseResult.sectionDescription = testCase.section?.description;
        if (testCase?.id) testCaseResult.defect = (await this.testCaseService.findOne({ id: testCase?.id }))?.defect;
        const testCaseResultData = await this.testCaseResultRepository.save(testCaseResult);
    ...
    async editTestCaseResult(editTestResultDto: EditTestResultDto, 
        testResultId: string, user: UserEntity
    ): Promise<TestCaseResultEntity> {
        const testCaseResult = await this.testCaseResultRepository
            ...
            .getOne();
        if (!testCaseResult) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        ...
        if (editTestResultDto?.status) {
            ...
            await this.activityService.createTestCaseResultActivity(testCaseResult.status, 
                ActivityAction[testCaseResult.status], testCaseResult, testSuite,
                testSuite?.project, user
            );
        }
        if (editTestResultDto?.comment) {
            testCaseResult.comment = editTestResultDto.comment;
            await this.activityService.createTestCaseResultActivity("COMMENT", ActivityAction.COMMENT,
                testCaseResult, testSuite, testSuite?.project, user
            );
        }
        if (editTestResultDto?.image) testCaseResult.image = editTestResultDto.image;
        const testCaseResultData = await this.testCaseResultRepository.save(testCaseResult);
        await this.testSuiteService.editTestSuiteReport(testCaseResultData.testSuiteId, user);
        if (editTestResultDto?.addCommentJira)
            {await this.defectService.addDefectComment(testCaseResultData.defect, editTestResultDto, user);}
        return testCaseResultData;
    }
     ...
     */
     ...
     async getTestResult(
        ...
        const testResult = await this.testCaseResultRepository
            ...
            .where('testCaseResult.id = :testResultId', { testResultId })
            .getOne();
        if (!testResult) throw new NotFoundException("translations.RECORD_NOT_FOUND");
