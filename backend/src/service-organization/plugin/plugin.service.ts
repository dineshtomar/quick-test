...
export class PluginService {
    ...
    async addPluginConfig(
        ...
        const checkPluginConfig = await this.getJiraPluginConfigByOrgId(currentLoggedInUser?.organization?.id);
        if (checkPluginConfig?.isIntegrated) throw new BadRequestException("translations.DUPLICATE_PLUGIN_CONFIG");
        
        const verifyPluginCredential = await this.verifyPlugin(addPluginConfigDto);
        if (!verifyPluginCredential) throw new BadRequestException("translations.INVALID_PLUGIN_CREDENTIALS");
    ...
    async getJiraPluginConfigByOrgId(
        ...
        const pluginConfig = await this.pluginConfigRepository
            ...
            .innerJoinAndSelect("pluginConfig.organization", "organization", 
                "organization.id = :id", { id: organizationId
            })
    ...
    async verifyJiraPlugin(credentials) {
        ...
        const authHeader = Buffer.from(`${userName}:${accessToken}`).toString('base64');
        let status = null;
        try {
            const response = await axios.get(
                `${webAddress}rest/agile/latest/board`, {
                method: 'GET',
                headers: {
    ...
    async updatePluginConfig(
        ...
        if (!checkPluginConfig) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        if (checkPluginConfig?.organization?.id !== currentLoggedInUser?.organization?.id) 
            {throw new ForbiddenException("translations.ACCESS_DENIED");}

        if (updatePluginConfigDto.accessToken) checkPluginConfig.accessToken = updatePluginConfigDto.accessToken;
        if (updatePluginConfigDto.webAddress) checkPluginConfig.webAddress = updatePluginConfigDto.webAddress;
        if (updatePluginConfigDto.userName) checkPluginConfig.userName = updatePluginConfigDto.userName;
        
        const verifyPluginCredential = await this.verifyPlugin(checkPluginConfig);
        if (!verifyPluginCredential) throw new BadRequestException("translations.INVALID_PLUGIN_CREDENTIALS");

        const updatedPluginConfig = await this.pluginConfigRepository.save(checkPluginConfig);
    ...
    async getPluginProjects(
        ...
        const pluginConfig = await this.getJiraPluginConfigByOrgId(currentLoggedInUser?.organization?.id);
        if (!pluginConfig?.isIntegrated) throw new BadRequestException("translations.INVALID_PLUGIN_CREDENTIALS");
    ...
    async getAllIssueKeysInPluginProject(
        ...
        if (!projectKey) throw new BadRequestException("translations.PROJECT_KEY_REQUIRED");

        const pluginConfig = await this.getJiraPluginConfigByOrgId(currentLoggedInUser?.organization?.id);
        if (!pluginConfig?.isIntegrated) throw new BadRequestException("translations.INVALID_PLUGIN_CREDENTIALS");

        const response = await this.getJiraIssueKeysByProjectKey(projectKey, pluginConfig);
        const issues: any = [];
        response?.issues?.forEach(issue => { 
            if (subtask || !issue?.fields?.parent) issues.push({ "key": issue?.key, "id": issue?.id }); 
        });
        return { issues }
    }
     ...
     */
    async getJiraIssueKeysByProjectKey(projectKey: string, credentials: PluginConfigEntity) {
        const { accessToken, webAddress, userName } = credentials;
        const authHeader = Buffer.from(`${userName}:${accessToken}`).toString('base64');
        let status = null;
        const max = 1000;
        ...
        try {
            const response = await axios.post(
                `${webAddress}rest/api/latest/search`, bodyData, {
                method: 'POST',
                headers: {
                    'Authorization': `Basic ${authHeader}`,
                    'Accept': 'application/json',
    ...
    async getAllIssueTypesInPluginProject(
        ...
        if (!projectId) throw new BadRequestException("translations.PROJECT_ID_REQUIRED");
     ...
     */
    async getJiraIssueTypesByProjecId(projectId: string, credentials: PluginConfigEntity) {
        const { accessToken, webAddress, userName } = credentials;
        const authHeader = Buffer.from(`${userName}:${accessToken}`).toString('base64');
        let status = null;
        try {
     ...
     */
    async getAllUsersInJiraProject(projectId: string, credentials: PluginConfigEntity) {
        const { accessToken, webAddress, userName } = credentials;
        const authHeader = Buffer.from(`${userName}:${accessToken}`).toString('base64');
        let status = null;
        const max = 1000;
     ...
     */
    async getSprintsFromJiraProject(projectId: string, credentials: PluginConfigEntity) {
        const sprints = [];
        ...
        boards?.values?.forEach(board => { 
            if (board?.location?.projectId === parsedProjectId) boardIds.push(board.id)
        });
        if (boardIds.length) {
    ...
    async addJiraIssue(payload, credentials) {
        ...
        let message = null;
        try {
            const response = await axios.post(`${webAddress}rest/api/latest/issue`, 
                bodyData, {
                method: 'POST',
                headers: {
                    'Authorization': `Basic ${authHeader}`,
            ...
            else if(errorData?.errors) message = Object.values(errorData?.errors)[0];
        }
    ...
    async getJiraSprintCustomField(credentials) {
        ...
        try {
            const response = await axios.get(
                `${webAddress}rest/api/latest/field`, {
                method: 'GET',
                headers: {
                    'Authorization': `Basic ${authHeader}`,
                    'Accept': 'application/json'
            ...
            for(const field of fields) if (field?.name?.toLowerCase() === "sprint") return field.id;
        } catch(error) { 
            status = error?.response?.status;
        }
        if (status && status === HttpStatus.UNAUTHORIZED) 
    ...
    async setCreateJiraIssuePayload(payload, credentials) {
        ...
        if (!payload?.parentId && payload?.sprintId) {
            const sprintCustomField = await this.getJiraSprintCustomField(credentials);
            if (sprintCustomField && Number(payload.sprintId)) fields[sprintCustomField] = Number(payload.sprintId);
        }
    ...
    async addCommentInJiraIssue(issueId: string, commentDoc, credentials) {
        ...
        let status = null;
        if (commentDoc?.image) {
            const res = await this.attachImageInJiraIssue(commentDoc.image, issueId, credentials);
            commentDoc.imageId = res?.length ? res[0].id : null;
            commentDoc.imageName = res?.length ? res[0].filename : null;
        }  
        const bodyData = await this.setAddCommentJiraIssuePayload(commentDoc, credentials);
        try {
            const response = await axios.post(
                `${webAddress}rest/api/latest/issue/${issueId}/comment`, bodyData, {
                method: 'POST',
                headers: {
                    'Authorization': `Basic ${authHeader}`,
                    'Accept': 'application/json',
     ...
     */
    async attachImageInJiraIssue(imageLink: string, issueId: string, credentials) {
        ...
        try {
            const response = await axios.post(
                `${webAddress}rest/api/2/issue/${issueId}/attachments`, form, {
                method: 'POST',
                headers 
            });
            if (response?.status === HttpStatus.OK) return response.data;
