...
export class SectionService {
    ...
    async getSectionsWithTestcases(
        ...
        const sections = await this._sectionRepository
            ...
            .select()
            .leftJoinAndSelect("sections.testcases", "testcases", "testcases.deletedAt IS NULL")
    ...
    async addSection(
        ...
        if (project.sections?.length && project.sections[0].name === createSectionDto.name) {
    ...
    async editSection(
        ...
        if (section.name === 'Unassigned') throw new ForbiddenException("translations.DEFAULT_SECTION");
        const updateDoc: any = {};
        if (editSectionDto.name) {
            updateDoc.name = UtilsService.titleCase(editSectionDto.name);
            if (updateDoc.name === "Unassigned") throw new ForbiddenException("translations.DEFAULT_SECTION"); 
            const checkDuplicateSection = await this._sectionRepository
                ...
                .getOne();
            if (checkDuplicateSection) throw new BadRequestException("translations.DUPLICATE_RECORD");
        ...
        const res = await this._sectionRepository.update( { id: section.id }, updateDoc);
        if (!res?.affected) throw new ExpectationFailedException("translations.RECORD_UPDATE_FAILED");
        return section;  
    }
    ...
    async checkSectionForProject(
        ...
        const section = await this._sectionRepository
            ...
            .getOne();
        if (!section) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        if (section.projectId !== projectId) throw new ForbiddenException("translations.ACCESS_DENIED");
        return section;
    }
    ...
    async getSectionsByProjectIdAndSectionId(
        ...
        const sections = await this._sectionRepository
            .createQueryBuilder("section")
            .innerJoinAndSelect("section.project", "project", "project.id = :projectId", {
                projectId
            })
            .leftJoinAndSelect("section.testcases", "testcases", "testcases.deletedAt is NULL")
            ...
            .addOrderBy("testcases.priority")
            .getMany();
        if(!sections.length) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        return sections.toDtos();
    }
    ...
    async deleteSection(
        ...
        const response = await this.testCaseService.deleteTestCasesBySection(section.id);
