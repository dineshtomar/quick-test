...
export class OrganizationService {
     ...
     */
    findOne(findData: FindOptionsWhere<OrganizationEntity>): Promise<OrganizationEntity> {
        return this.organizationRepository.findOneBy(findData);
    }

    async changeOrganizationName(organizationId: string, newOrganizationName: string, currentLoggedInUser: UserEntity): Promise<boolean> {
    ...
    async updateStripeCustomer(organizationId: string, customerId: string): Promise<boolean> {
        const checkUpdate = await this.organizationRepository.update(
            {
                id: organizationId,
            },
    ...
    async updateOrgSubscriptionStatus(organizationId: string, subscriptionStatus: OrgSubscriptionStatus) {
        const checkUpdate = await this.organizationRepository.update(
            {
                id: organizationId,
            },
    ...
    async findByOrganizationByIdOrName(options: Partial<{ organizationname: string; id: string }>): Promise<OrganizationEntity | undefined> {
        ...
        if (options.organizationname) {
            queryBuilder.orWhere("organization.organizationname = :organizationname", {
     ...
     * Create organization
     */
    async createOrganization(organizationRegisterDto: CreateOrganizationDto): Promise<OrganizationEntity> {
        const newOrganization: CreateOrganizationDto = organizationRegisterDto;
        newOrganization.name = UtilsService.sentenceCase(organizationRegisterDto.name);
        const organization = this.organizationRepository.create(newOrganization);

        const organizationsData = await this.organizationRepository.save(organization);
     ...
     */
    async getOrganizations(pageOptionsDto: OrganizationsPageOptionsDto, user: UserEntity): Promise<OrganizationsPageDto> {
        const queryBuilder = this.organizationRepository.createQueryBuilder("organization");
        ...
        queryBuilder.leftJoinAndSelect("organization.projects", "projects").where("projects.deleted_At is Null");

        const [organizations, organizationsCount] = await queryBuilder.skip(pageOptionsDto.skip).take(pageOptionsDto.take).getManyAndCount();
     ...
     */
    async addMemberInOrganization(addOrganizationMemberDto: AddOrganizationMemberDto, currentLoggedInUser: UserEntity, lang: string): Promise<UserEntity> {
        const tempPassword = UtilsService.generateRandomString(8);
        const user = await this.userCreateService.addUserInOrganization(addOrganizationMemberDto, tempPassword, currentLoggedInUser.organization, currentLoggedInUser, lang);
        if (user) {
            return user;
        }
     ...
     */
    async getAllMembers(pageOptionsDto: MembersPageOptionsDto, currentLoggedInUser: UserEntity): Promise<MembersPageDto> {
        const members = await this.userReadService.getAllMemebersInOrganization(pageOptionsDto, currentLoggedInUser.organization.id, currentLoggedInUser);
        return members;
    }
     ...
     */
    async getArchivedMembers(pageOptionsDto: MembersPageOptionsDto, currentLoggedInUser: UserEntity): Promise<MembersPageDto> {
        const members = await this.userReadService.getArchivedMemebersInOrganization(pageOptionsDto, currentLoggedInUser.organization.id, currentLoggedInUser);
        return members;
    }
     ...
     */
    async updateMemberInOrganization(updateOrganizationMemberDto: UpdateOrganizationMemberDto, memberId: string, currentLoggedInUser: UserEntity, lang: string): Promise<UserEntity> {
        const member = await this.userUpdateService.updateMemberInOrganization(updateOrganizationMemberDto, memberId, currentLoggedInUser, lang);
        if (member) {
            return member;
        }
     ...
     */
    async resendResetPasswordLink(memberId: string, currentLoggedInUser: UserEntity, lang: string): Promise<boolean> {
        ...
        const checkSameOrganization = member.organization.id === currentLoggedInUser.organization.id;
        if (!checkSameOrganization) throw new ForbiddenException("translations.ACCESS_DENIED");
     ...
     */
    async addMultipleMembersInOrganization(addMultipleMembersOrganizationDto: AddMultipleMembersOrganizationDto, currentLoggedInUser: UserEntity, lang: string): Promise<any> {
        ...
        for (let i = 0; i < addMultipleMembersOrganizationDto.members.length; i++) {
            ...
            const user = await this.userCreateService.addUserInOrganization(addMultipleMembersOrganizationDto.members[i], tempPassword, currentLoggedInUser.organization, currentLoggedInUser, lang);
            if (user) {
                members.push(user);
            }
        }
     ...
     */
    async getOrganization(currentLoggedInUser: UserEntity, projectId: string): Promise<any> {

        const projectMembers = await this.projectMembersRepository
            .createQueryBuilder("project_member")
            .where('project_member."organization_id" =:organizationId', {organizationId : currentLoggedInUser.organization.id})
            .andWhere('project_member."project_id" =:projectId', {projectId})
            .getMany();

        if (!projectMembers) throw new NotFoundException("translations.RECORD_NOT_FOUND");
     ...
     */
    async getAllProjects(currentLoggedInUser: UserEntity): Promise<ProjectFavoriteDetailDto[]> {
        const projects = await this.projectReadService.getAllProjects(currentLoggedInUser);
        return projects;
    }
    ...
    async getArchivedProjects(currentLoggedInUser: UserEntity): Promise<ProjectEntity[]> {
        const projects = await this.projectReadService.getArchivedProjects(currentLoggedInUser);
        return projects;
    }
     ...
     */
    async getMostActiveProjects(pageOptionsDto: ProjectListPageOptionsDto, currentLoggedInUser: UserEntity) {
        const result = await this.projectReadService.getMostActiveProjects(pageOptionsDto, currentLoggedInUser);
        return result;
    }
     ...
     */
    async getSearchResults(searchOptionsDto: SearchOptionsDto, currentLoggedInUser: UserEntity): Promise<any> {
        ...
        const projects = await this.projectReadService.searchProjects(query, currentLoggedInUser?.organization?.id);
        ...
        const testSuites = await this.testSuiteService.searchTestSuites(query, currentLoggedInUser);
        if (testSuites.length) res.testSuites = testSuites;

        const milestones = await this.milestoneService.searchMilestones(query, currentLoggedInUser);
    ...
    filterExactSearchResults(result: any, query: string) {
        ...
        if (result.testCases) {
            ...
            filteredResult.testCases = filteredTestCasesById.concat(filteredTestCasesByTitle);
        }
    ...
    async findTrialExpiredOrgs() {
        const date = new Date();
        const trialStartDate = date.setDate(date.getDate() - Number(FreeTrial.TRIAL_DAYS_PLUS_ONE));
        const trialEndDate = date.setDate(date.getDate() + 1);
        return this.organizationRepository.find({
            where: [
                { freeTrialStartDate: LessThan(new Date(trialStartDate)), subscriptionStatus: OrgSubscriptionStatus.freeTrial },
                { freeTrialStartDate: Between(new Date(trialStartDate), new Date(trialEndDate)), subscriptionStatus: OrgSubscriptionStatus.freeTrial },
            ],
        });
    }
    ...
    findUpcomingTrialExpiringOrgs() {
        const date = new Date();
        const upcomingTrialStartDate = date.setDate(date.getDate() - Number(FreeTrial.UPCOMING_FREETRIAL_ENDING_PLUS_ONE));
        const UpcomingTrialEndDate = date.setDate(date.getDate() + 1);
        return this.organizationRepository.find({
            where: {
                freeTrialStartDate: Between(new Date(upcomingTrialStartDate), new Date(UpcomingTrialEndDate)),
                subscriptionStatus: OrgSubscriptionStatus.freeTrial,
            },
        });
    }

    async getProjectUnAssignedMember(organizationId: string, projectId:string): Promise<unknown> {
        return await this.userReadService.getProjectUnAssignedMember(organizationId, projectId)
    }

    async addProjectMember(organizationId: string, addMemberDto:AddMemberDto): Promise<unknown> {
        return await this.userCreateService.addProjectMember(organizationId, addMemberDto)
    }

    async getProjectAssignedMember(organizationId: string, projectId:string): Promise<unknown> {
        return await this.userReadService.getProjectAssignedMember(organizationId, projectId)
    }

    async projectMemberDelete(organizationId: string, projectId:string,userId:string) {
        return await this.userDeleteService.deleteProjectMember(organizationId, projectId, userId)
    }
}
