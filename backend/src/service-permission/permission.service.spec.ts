    ...
    */ 
    ...
    describe('findAllPermissions', () => {
        it("should return all permissions", async () => {
            mocked(permissionService.findAllPermissions).mockImplementation(async () => Promise.resolve((mockedPermission as unknown) as Permission[]))
            return await permissionService.findAllPermissions(mockedPermission).then((data) => {
    ...
    */ 
    ...
    describe("findAllPermissionsByRoleId", () => {
        it("should return all the psrmissions for specific ID", async () => {
            mocked(permissionService.findAllPermissionsByRoleId).mockImplementation(async() => 
                await Promise.resolve((mockedPermission as unknown) as Permission[])
            );    
    
            await expect(
                permissionService.findAllPermissionsByRoleId(
                    
                        mockedPermission.roleId,
