...
export class AuthService {
    ...
    async createToken(user: UserEntity | UserDto): Promise<TokenPayloadDto> {
        return new TokenPayloadDto({
            expiresIn: this.configService.get("app.jwtExpirationTime", {infer: true}),
    ...
    async validateUser(userLoginDto: UserLoginDto): Promise<UserEntity> {
        const user = await this.userReadService.findUserByEmail(userLoginDto.email, ['role', 'organization']);
        const isPasswordValid = await UtilsService.validateHash(
            userLoginDto.password,
            user && user.password
        );
        if (!user || !isPasswordValid) {
            throw new ExpectationFailedException("translations.INCORRECT_CREDENTIALS");
        }
    ...
    async sendWelcomeNewUserEmail(
        ...
        const emailVerifyToken = await this.userCreateService.createEmailVerificationToken(user.email); 
    ...
    async sendWelcomeMemberEmail(
        ...
        this.mailService.sendWelcomeMemberEmail(member, requestedBy, resetLink, lang);
        return true;
    }
    ...
    async sendContactFormAcknowledgementEmail(
        ...
        const name = data.lastName 
            ...
            : data?.firstName;
        this.mailService.sendContactFormAcknowledgementEmail(data?.email, name, lang);
        return true;
    }
    ...
    async resendEmailVerificationLink(
        ...
        if (!user) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        if (user.isVerified) throw new ForbiddenException("translations.ALREADY_VERIFIED");
        const emailVerifyToken = await this.userCreateService.createEmailVerificationToken(
            user.email
        );
        const verifyLink = `${this.webUrl}verify?token=${emailVerifyToken.token}`;
