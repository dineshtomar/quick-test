...
import { contextMiddleware , RawBodyMiddleware , JsonBodyMiddleware } from "./middlewares";
...
@Module({
    imports: [
        ConfigModule.forRoot({
            isGlobal: true,
            load: [appConfig, awsConfig, databaseConfig, emailConfig, pdfConfig, stripeConfig],
            envFilePath: ['.env'],
        }),
        ...
        I18nModule.forRootAsync({
            useFactory: (configService: ConfigService<AllConfigType>) => ({
                fallbackLanguage: configService.get('app.fallbackLanguage', {infer: true}),
                loaderOptions: {
                    path: path.join(process.cwd(), "src","i18n/"),
                    watch: configService.get('app.nodeEnv', {infer: true}) === 'developmet',
                },
            }),
            resolvers: [
                {
