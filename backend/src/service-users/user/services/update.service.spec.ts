...
describe("UserUpdateService", () => {
    ...
    describe("updateUserStatus", () => {
        it("should update the user status", async() => {
            mocked(userUpdateService.updateUserStatus).mockImplementation(() => Promise.resolve(mockedUser))
            expect(
        ...
        it("should throw error if status is not boolean", () => {
            mocked(userUpdateService.updateUserStatus).mockImplementation(() => Promise.reject(new Error('User status can be boolean type only')))
            expect(
                userUpdateService.updateUserStatus(
                  { status: null },
                  mockedUser.id,
     ...
     */
     ...
     describe("reactivateUser", () => {
        ...
        it("should return error if reactivate user fails", async () => {
            mocked(userUpdateService.reactivateUser).mockImplementation(() =>
                Promise.reject(
                    new Error("User can not be reactivated")
                )
            );
            expect(
                userUpdateService.reactivateUser("6409d23b-44f8-455e-b5ff-0da8a30a3f7c",mockedUser)
