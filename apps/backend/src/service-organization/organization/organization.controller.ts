...
import { ApiBearerAuth, ApiResponse, ApiTags, ApiOkResponse } from "@nestjs/swagger";
...
export class OrganizationController {
    ...
    async addMemberInOrganization(
        ...
        const organizations = await this._organizationService.addMemberInOrganization(
    ...
    async getArchivedMembers(
        ...
        return new ResponseSuccess("translations.LIST_ARCHIVED_MEMBERS", organizations);
    }
    ...
    async getOrganization(
        ...
        return new ResponseSuccess("translations.ORG_DETAILS_WITH_LISTING", organizations);
    }
    ...
    async resendResetPasswordLink(
        ...
        return new ResponseSuccess(
            "translations.RESET_PASS_LINK_SENT",
            {}
        );
    }
    ...
    async addMultipleMembersInOrganization(
        @Req() request: Request,
        @Body() addMultipleMembersOrganizationDto: AddMultipleMembersOrganizationDto,
        @AuthUser() user: UserEntity,
    ): Promise<ResponseSuccess> {
        const lang = request.headers['accept-language'];
        const res = await this._organizationService.addMultipleMembersInOrganization(
    ...
    @ApiResponse({
        status: HttpStatus.OK,
        description: "Get all projects in the organization with total active test runs and milestones",
        type: ProjectDetailsDto
    })
    async getAllProjects(
        @AuthUser() user: UserEntity
    ): Promise<ResponseSuccess> {
        const res = await this._organizationService.getAllProjects(
            user
        );
    ...
    async getArchivedProjects(
        ...
        const res = await this._organizationService.getArchivedProjects(
            user
        );

        return new ResponseSuccess("translations.ARCHIVED_PROJECT_LIST", res.toDtos()); 
    }
    ...
    @ApiResponse({
        status: HttpStatus.OK,
        description: "Get most active projects in the organization with total test changes"
    })
    async getMostActiveProjects(
        @Query(new ValidationPipe({ transform: true })) pageOptionsDto: ProjectListPageOptionsDto,
        @AuthUser() user: UserEntity
    ): Promise<ResponseSuccess> {
    ...
    @ApiResponse({
        status: HttpStatus.OK,
        description: "Get TestCase, TestRuns, Milestones and projects search results"
    })
    ...
    async getUnassignedMembers(
        ...
        const res = await this._organizationService.getProjectUnAssignedMember(user.organization_id,projectId);
    ...
    async addProjectMember(
        ...
        const res = await this._organizationService.addProjectMember(user.organization_id,addMemberDto);
        return new ResponseSuccess("Organizaton members", res);
    }
    ...
    async getOrganizationUser(
        ...
        const res = await this._organizationService.getProjectAssignedMember(user.organization_id,projectId);
        return new ResponseSuccess("Organizaton members", res);
    }
    ...
    async RemoveProjectMember(
        ...
        const deleted = await this._organizationService.projectMemberDelete(user.organization_id, projectId, userId);
