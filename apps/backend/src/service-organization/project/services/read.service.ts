...
export class ProjectReadService {
    ...
    async checkProjectWithSameName(
        ...
        const checkProjectWithSameName = await this.projectRepository
            ...
            .getOne();
        if (checkProjectWithSameName) throw new ConflictException("translations.DUPLICATE_RECORD");
        return true;
    }
    ...
    async getTestCase(
        ...
        return this.testCaseService.getTestCase(
            testCaseId,
        );
    }
    ...
    async getTestSuiteDetail(
        ...
        const testSuites = await this.testSuiteService.getTestSuiteDetail(
            testSuiteId,
        );
    ...
    async getProjectTodo(
        ...
        const todoList = await this.testSuiteService.getTodoList(
            projectId
        );
        return todoList;
    }
     ...
     */
     ...
     async getOpenMilestones(
        ...
        const milestone = await this.milestoneService.getOpenMilestones(
            projectId,
        );
        return milestone;
    }
     ...
     */
     ...
     async getAllMilestones(
        ...
        const milestone = await this.milestoneService.getAllMilestones(
            projectId,
        );
        return milestone;
    }
    ...
    async getAllProjects(
        ...
        const queryBuilder =  this.projectRepository
            .createQueryBuilder("project")
            .innerJoinAndSelect("project.organization", "organization", "organization.id = :id", {
                id: currentLoggedInUser.organization.id
            })
            .leftJoinAndSelect("project.testsuites", "testsuites", "testsuites.deletedAt is NULL")
            .leftJoinAndSelect("project.milestones", "milestones", "milestones.deletedAt is NULL")
        ...
        if(currentLoggedInUser.roleId == RoleId.MEMBER){
            ...
            queryBuilder.andWhere(`EXISTS (SELECT 1 FROM project_members p WHERE p.project_id = project.id AND p.user_id = '${currentLoggedInUser.id}')`);
        }
        ...
        const projectList = await this.projectUpdateService.markFavoriteProject(projects, currentLoggedInUser);
        return projectList;
    }
    ...
    async getMostActiveProjects(
        ...
        const projectList = await this.findAllProjectsWithTestChanges(pageOptionsDto, currentLoggedInUser);
        ...
        const testChangesActivities = await this.activityService.getTestCaseChangesActivitiesByProjects(
            projectIds, pageOptionsDto
        );
        const result = [];
        ...
        for(let i = 0; i < testChangesActivitiesLength; i++) {  
            ...
            result.push(this.projectCreateService.addInactiveProjects(projects, activeProjects));
        }
        return result;
    } 
    ...
    async findAllProjectsWithTestChanges(
        ...
        for( let i = 0; i < projects.length; i++) {
            ...
            project.totalTestChanges = await this.activityService.getProjectsTestChangesCount(
                projects[i].id,
                date
            );
     ...
     */
     async getTestCaseByInternalId(
        ...
        return this.testCaseService.getTestCaseByInternalId(
            projectId,
            testCaseId,
        );
    }
    ...
    async searchProjects(
        ...
        const projects = await this.projectRepository
            .createQueryBuilder("project")
            .innerJoinAndSelect("project.organization", "organization", "organization.id = :organizationId", {
                organizationId
            })
