     ...
     */
     ...
     describe("findOne", () => {
        it("should find one Test-case", async() => {
            mocked(testCaseService.findOne).mockImplementation(async () => await Promise.resolve((mockTestCase as unknown) as TestCaseEntity))
    ...
    */ 
    ...
    describe("findBytestByIdOrName", () => {
        it("should return a single test, searched by name", async () => {
            mocked(testCaseService.findByTestCaseByIdOrName).mockImplementation(async() => 
            ...
            it("should return a single test-case, searched by id", async () => {
                mocked(testCaseService.findByTestCaseByIdOrName).mockImplementation(async() => 
                    await Promise.resolve(mockTestCase)
                );    
    ...
    describe("edit", () => {
        it("should return error if test case not found.", async () => {
            mocked(testCaseService.findOne).mockImplementation(() =>
                Promise.reject(
                    new Error("TestCase details could not be fetched.")
                )
            );
            expect(
                testCaseService.findOne({ id: mockTestCase.id })
            ).rejects.toThrow("TestCase details could not be fetched.");
        });
    ...
    describe("getTestCase", () => {
        ...
        it("should return error if TestCase details could not be fetched.", async () => {
            ...
            expect(
                testCaseService.getTestCase(mockTestCase.id)
            ).rejects.toThrow("TestCase details could not be fetched.");
        });
    });
    ...
    describe("reOrderPriorities", () => {
        it("should return error if testcase and section does not match", async () => {
            ...
            expect(
                testCaseService.reOrderPriorities(
                    mockTestCase.id,
                    mockSection.id,
                    5
                )
    ...
    describe("deleteTestCase", () => {
        ...
        it("should return error if TestCase could not be deleted.", async () => {
            mocked(testCaseService.deleteTestCase).mockImplementation(() =>
                Promise.reject(
                    new Error("TestCase could not be deleted.")
                )
            );
            expect(
                testCaseService.deleteTestCase(mockTestCase.id)
            ).rejects.toThrow("TestCase could not be deleted.");
        });
    });
    ...
    describe("searchTestCases", () => {
        it("should return empty array if not title and id provided", async () => {
            ...
            expect(
                testCaseService.searchTestCases(
                    null,
                    null,
                    mockedUser
                )
