...
export class MilestoneService {
    ...
    async findMilestoneById(
        ...
        const milestone = await queryBuilder.where('milestone.id = :id', { id }).getOne();
        return milestone;
    }
     ...
     */
     ...
     async createMilestone(
        ...
        createMilestoneDto.name = UtilsService.titleCase(
        ...
        await this.activityService.createMilestoneActivity("Created", milestone, project, user);
    ...
    async updateMilestoneStatus(
        ...
        await this.activityService.createMilestoneActivity("COMPLETED", updatedMilestone, 
            milestone.project, currentLoggedInUser
        );
    ...
    async editMilestone(
        ...
        const milestone = await this.milestoneRepository.findOneBy({ id: milestoneId });
        if (!milestone) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        if(editMilestoneDto.name){           
            milestone.name = UtilsService.titleCase(
                editMilestoneDto.name
            );
        }
    ...
    async searchMilestones(
        ...
        const milestones = await this.milestoneRepository
            ...
            .innerJoinAndSelect("milestones.project", "project", "project.id IN (:...projectIds)", {
                projectIds
            })
            ...
            .getMany();
        const milestonesDetails = milestones.map(milestone => new MilestoneDetailsDto(milestone));
