...
export class TestSuiteService {
     ...
     * Create Test Suite
     */
    async createTestSuite(createTestSuiteDto: CreateTestSuiteDto,
        project: ProjectEntity, user: UserEntity
    ): Promise<TestSuiteEntity> {
        if (!project) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        createTestSuiteDto.name = UtilsService.titleCase(
            createTestSuiteDto.name
        );
        const testSuite = this.testSuiteRepository.create(createTestSuiteDto);
        const testCases = await this.testCaseService.getTestCasesSortedBySection(project.id);
        const testCaseSize = testCases.length;
        if (!testCaseSize) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        let ind = -1;
        for(let i = 0; i < testCaseSize; i++) {
            if (testCases[i].section?.name === "Unassigned") ind = i;
            else break;
        }
        const testCasesSorted: TestCaseEntity[] = testCases.slice(ind+1, testCases.length);
        ...
        if(createTestSuiteDto.milestone) {
            await this.milestoneInTestSuite(
                testSuite,
                createTestSuiteDto.milestone
            );
        }
        const testSuiteData = await this.testSuiteRepository.save(testSuite);
        await this.postCreateTestSuiteHandle(testSuiteData, project, user, testCaseSize, testCasesSorted);
     ...
     * creation tasks  
     */
    async postCreateTestSuiteHandle(testSuiteData: TestSuiteEntity, project: ProjectEntity, 
        user: UserEntity, testCaseSize: number, testCasesSorted:  TestCaseEntity[]
    ) {
        await this.activityService.createTestSuiteActivity("Created", testSuiteData, project, user);
    ...
    async createFilteredTestSuite(
        ...
        project: ProjectEntity, currentLoggedInUser: UserEntity
    ): Promise<TestSuiteEntity> {
        createFilteredTestSuiteDto.name = UtilsService.titleCase(createFilteredTestSuiteDto.name);
        const testSuite = this.testSuiteRepository.create(createFilteredTestSuiteDto);
        const testCases = await this.testCaseService.getTestCasesByIds(project,createFilteredTestSuiteDto.testCaseIds);
        testSuite.project = project;
        testSuite.user = currentLoggedInUser;
        if(createFilteredTestSuiteDto.assignTo) {
            await this.assignUserTestSuite(testSuite, createFilteredTestSuiteDto.assignTo,
                currentLoggedInUser.organization.id);
        }
        if(createFilteredTestSuiteDto.milestone) {
            await this.milestoneInTestSuite(testSuite, createFilteredTestSuiteDto.milestone);
        }
        const testSuiteData = await this.testSuiteRepository.save(testSuite);
        
        await this.postCreateTestSuiteHandle(testSuiteData, project, currentLoggedInUser, 
    ...
    async assignUserTestSuite(
        ...
        const assignedUser = await this.userReadService.findUserById(userId, 'organization');
        if(!assignedUser) throw new NotFoundException("translations.RECORD_NOT_FOUND");
     ...
     */
     async milestoneInTestSuite(
        ...
        const milestone = await this.milestoneService.findMilestoneById(milestoneId, 'project');
        if(!milestone) throw new NotFoundException("translations.RECORD_NOT_FOUND");
    ...
    async editTestSuiteReport(
        ...
        const testSuiteReport = await this.testSuiteReportRepository
            ...
            .getOne();
        if (!testSuiteReport) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        ...
        const updateReport = await this.testSuiteReportRepository.save(testSuiteReport);
        let suiteStatus: TestSuiteStatus = TestSuiteStatus.PENDING;
        let completedAt;
        if (updateReport.untested < updateReport.total && updateReport.untested !== 0) {
            suiteStatus = TestSuiteStatus.INPROGRESS;
        } else if (updateReport.untested === 0) {
            suiteStatus = TestSuiteStatus.COMPLETED;
            completedAt = new Date();

            const testSuite = await this.testSuiteRepository.createQueryBuilder('testsuite')
                .leftJoinAndSelect('testsuite.project', 'project')
                .where('testsuite.id = :testSuiteId', { testSuiteId })
                .getOne();
            await this.activityService.createTestSuiteActivity("Completed", testSuite, testSuite?.project, user); 
        }
        await this.testSuiteRepository.createQueryBuilder()
    ...
    async getTestSuiteResultCounts(testSuiteId: string) {
        const testSuite = await this.testSuiteRepository.createQueryBuilder("testsuite")
            .where("testsuite.id = :testSuiteId", { testSuiteId })
            .loadRelationCountAndMap("testsuite.passedCount", "testsuite.testresults",
                "testresults",
                (qb) => qb.where("testresults.status = :status", { status: "PASSED" })
            )
            .loadRelationCountAndMap("testsuite.failedCount", "testsuite.testresults",
                "testresults",
                (qb) => qb.where("testresults.status = :status", { status: "FAILED" })
            )
            .loadRelationCountAndMap("testsuite.blockedCount", "testsuite.testresults",
                "testresults",
                (qb) => qb.where("testresults.status = :status", { status: "BLOCKED" })
            )
            .loadRelationCountAndMap("testsuite.retestCount", "testsuite.testresults",
                "testresults",
                (qb) => qb.where("testresults.status = :status", { status: "RETEST" })
            )
            .loadRelationCountAndMap("testsuite.untestedCount", "testsuite.testresults",
                "testresults",
                (qb) => qb.where("testresults.status = :status", { status: "UNTESTED" })
            )
            .getOne();
        return testSuite;    
    }
    ...
    async editTestSuite(
        ...
        const testSuite = await this.testSuiteRepository
            ...
            .where('testsuite.id = :testSuiteId', { testSuiteId })
            .getOne();
        if (!testSuite) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        if (editTestSuiteDto?.name) testSuite.name = UtilsService.titleCase(editTestSuiteDto.name);
        if (editTestSuiteDto?.description) testSuite.description = editTestSuiteDto.description;
        if (editTestSuiteDto?.status) testSuite.status = editTestSuiteDto.status;

        if(editTestSuiteDto?.assignTo && testSuite.assignedTo.id !== editTestSuiteDto?.assignTo) {
            await this.assignUserTestSuite(testSuite, editTestSuiteDto.assignTo, user.organization.id);
        }        
     ...
     */
     ...
     async deleteTestSuite(
        ...
        if (!testSuite) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        const testSuitesActivites = await this.activityService.getTestSuitesActivitiesByTestSuitesId(testSuite.id);
    ...
    async getTodoList(
        ...
        const users: UserAssignedTestCasesDto[] = this.addTestCasesToAssignedUsers(testSuites);
        ...
        return new TestSuiteTodoListDto(
            users, 
            testSuiteList
        );
    }
        ...
        try{
            ...
            testSuites.forEach((testSuite) => {
                if(testSuite.assignedTo){
                    ...
                    const testSuiteCompletedTestCases = (testSuite.testreport.total - testSuite.testreport.untested);
                    ...
                    userAssignedTestCasesDto[userId].totalActiveTestCases += testSuiteActiveTestCases;
                    userAssignedTestCasesDto[userId].totalCompletedTestCases += testSuiteCompletedTestCases;
                }
            });
    ...
    async searchTestSuites(
        ...
        const testSuites = await this.testSuiteRepository
            .createQueryBuilder("testsuite")
            .innerJoinAndSelect("testsuite.project", "project", "project.id IN (:...projectIds)", {
                projectIds
            })
        ...
        const testSuitesDetails = testSuites.map(testSuite => new TestSuiteDetailDto(testSuite));
     ...
     async addTestSuitesPdf(
        ...
        if (testSuiteIds.length === 0) {
            ...
            testSuites = await this.testSuiteRepository
                ...
                .getMany();
            if(testSuites.length === 0) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        }
        const key = await this.pdfService.generateTestSuitesPdf(project, testSuites);
    ...
    async addTestSuiteResultPdf(
        ...
        const testSuite = await this.testSuiteRepository
            ...
            .getOne();
        if(!testSuite) throw new NotFoundException("translations.RECORD_NOT_FOUND");
        const testCaseResultsObject = this.groupTestCaseResultBySection(testSuite.testresults);
