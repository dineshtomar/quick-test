...
describe("DefectService", () => {
    ...
    describe("getDefectByKeyAndTestcaseId", ()=>{
        it("Get defects by key and testcase id",async () => {
            ...
            mocked(
                ...
                Promise.reject(new Error("Defect with this key and testcaseId  not found"))
            );
    ...
    describe("getIssueDetailsByKey", ()=>{
        it("Get issue details by defect key",async () => {
            ...
            mocked(
                defectService.getIssueDetailsByKey
            ).mockImplementation(() => 
                Promise.resolve(mockedDefect as DefectDto)
            );
    ...
    describe("getJiraIssueByKey", ()=>{
        it("Should return error if get jira issue fails", async () => {
            ...
            mocked(
                defectService.getJiraIssueByKey
            ).mockImplementation(() => 
                Promise.reject(new Error("Defect with this id not found"))
            );
    ...
    describe("addDefectComment", ()=>{
        it("Check the defect with orgId",async () => {
            ...
            Promise.resolve((mockedUser.organization.id as unknown) as PluginConfigEntity)
            );
            expect(
                pluginService.getJiraPluginConfigByOrgId(
                    mockedOrgId
                )
        ...
        expect(
            defectService.addDefectComment(
                mockedDefect,
                mockedComment,
                mockedUser,
            )
    ...
    describe("getDefectDetails", ()=>{
        it("Should return error for defect not found", async () => {
            ...
            mocked(
                defectService.getDefectDetails
            ).mockImplementation(() => 
        ...
        it("Get defect details",async () => {
            ...
            expect(await defectService.getDefectDetails(result.issueId, mockedUser)).toMatchObject(result)
        });
    });
});
