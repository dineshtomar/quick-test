...
export class DefectService {
     ...
     * Create new defect
     */
    async createDefect(pluginKey: string, pluginId: string,
    ...
    async updateDefectRefInTestCase(
        ...
        let defect = await this.getDefectByKeyAndTestcaseId(updateDefectRefDto.pluginKey, testCaseId);
        if (defect) return defect.toDto();

        const pluginConfig = await this.pluginService
            .getJiraPluginConfigByOrgId(currentLoggedInUser?.organization?.id);
        if (!pluginConfig) throw new BadRequestException("translations.INVALID_PLUGIN_CREDENTIALS");

        const issue: any = await this.getIssueDetailsByKey(updateDefectRefDto?.pluginKey, pluginConfig);
        defect = await this.createDefect(updateDefectRefDto.pluginKey, issue?.id, pluginConfig);
        const response = await this.testCaseService.addJiraReference(defect, testCaseId, currentLoggedInUser?.organization?.id);
    ...
    async getDefectByKeyAndTestcaseId(
        pluginKey: string, testCaseId: string
    ): Promise<DefectEntity> {
    ...
    async getJiraIssueByKey(pluginKey: string, credentials) {
        ...
        let status = null;
        try {
            const response = await axios.get(`${webAddress}rest/api/3/issue/${pluginKey}`, {
                method: 'GET',
                headers: {
    ...
    async addDefect(addDefectDto: AddDefectDto, currentLoggedInUser: UserEntity) {
        const credentials = await this.pluginService
            .getJiraPluginConfigByOrgId(currentLoggedInUser?.organization?.id);
        if (!credentials) throw new BadRequestException("translations.INVALID_PLUGIN_CREDENTIALS");
        const issue = await this.pluginService.addJiraIssue(addDefectDto, credentials);
        if (issue?.key && addDefectDto?.testCaseId) 
            {await this.updateDefectRefInTestCase(addDefectDto.testCaseId, { pluginKey: issue.key }, currentLoggedInUser);}
        return issue;
    }
    ...
    async addDefectComment(
        defect: DefectEntity, commentDoc, currentLoggedInUser: UserEntity 
    ) {
        const credentials = await this.pluginService
            .getJiraPluginConfigByOrgId(currentLoggedInUser?.organization?.id);
        const checkCommentDoc = commentDoc?.status || commentDoc?.comment || commentDoc?.image; 
        ...
        if (credentials && checkCommentDoc && checkDefect) {
            ...
            if (currentLoggedInUser.lastName) commentDoc.tester += ` ${currentLoggedInUser.lastName}`; 
            await this.pluginService.addCommentInJiraIssue(defect.pluginId, commentDoc, credentials);
        }
    }
     ...
     */
     findSprintsInIssue(issue) {
        ...
        for(const field of customFields) {
            if (Array.isArray(field) && field.length > 0 && field[0]?.id && field[0]?.boardId) 
                {return field;}
        }
    }
}
