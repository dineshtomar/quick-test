...
export class CronService {
    ...
    async handleCronJob() {
        const trialExpiredOrganizations = await this.organizationService.findTrialExpiredOrgs();
        trialExpiredOrganizations.forEach(async(org) => {
            await this.organizationService.updateOrgSubscriptionStatus(org.id, OrgSubscriptionStatus.cancelled);
        ...
        const trialPeriodExpiringOrgs = await this.organizationService.findUpcomingTrialExpiringOrgs();
